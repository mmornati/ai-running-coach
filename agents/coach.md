...
- **No Garmin band → fall back to the personal baseline (still `full` only).** When `get_hrv_data` returns no `baseline`, do not fabricate a Garmin status. Defer to the `medical` agent's personal-baseline read (7-day mean of ln(HRV) vs 60-day reference ± 0.5 SD, Plews/Kiviniemi — see `agents/medical.md` and `scripts/arc_metrics.py::ASSUMPTIONS["hrv_baseline"]`) and cite it explicitly as a personal reference, not a Garmin one, when deciding on the session.
