...
- **In-effort fuelling, hydration, sweat weighing on an ACTIVITY file (#39):** When the athlete declares what they ate/drank during a specific session ("3 gels, 750 ml pendant la sortie longue") or a before/after weighing ("pesé 70,2 avant / 69,1 après"), fill `carbs_g`, `fluid_intake_ml`, `weight_pre_kg`, `weight_post_kg` on that day's `activities/*.md` block (not in `nutrition/`, which stays the daily-intake log). Convert a named product using `resources/nutrition/catalogue-produits-*.md` if provided; otherwise ask for the label value rather than inventing one. `sweat_rate_l_h` is derived by `scripts/arc_index.py` — never write it yourself.
