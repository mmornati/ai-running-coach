...
- **Personal HRV baseline when Garmin's band is absent (`full` only):** `get_hrv_data` sometimes returns no `baseline` (new watch, insufficient Garmin-side history). Never invent a Garmin band in that case, and never report "balanced/low" against a band that was not actually returned. Instead, cite the workspace's own personal baseline: a 7-day rolling mean of ln(overnight HRV) compared to a 60-day reference ± 0.5 SD (Plews, Laursen & Buchheit 2013; Kiviniemi et al. 2007), computed from `medical/*_health.md` history and exposed by the dashboard (`/api/health` → `hrv_personal_low_ms`/`hrv_personal_high_ms`/`hrv_personal_status`; full method in `scripts/arc_metrics.py::ASSUMPTIONS["hrv_baseline"]`). Below 30 days of usable history the status is "en construction" (building up) — say so rather than guessing a band from too little data.
