...
- `_write_canonical_samples` (#42) normalise ensuite ces mêmes records (sans reparser le FIT) via `normalise_records` de `scripts/arc_samples.py` : mapping `heart_rate → hr_bpm`, `distance → distance_m`, `enhanced_altitude/altitude → altitude_m`, `enhanced_speed/speed → speed_ms` (déjà en m/s), `timestamp → t_s` relatif au départ, et **doublement de la cadence** (`cadence` FIT course à pied compte un seul pied/min, `cadence_spm` en sortie compte les deux) — voir la docstring du module pour le détail et les sources.
